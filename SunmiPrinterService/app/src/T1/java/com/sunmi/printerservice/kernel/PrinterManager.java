...
import static com.sunmi.printerservice.manager.PrinterState.R_HOTS;
import static com.sunmi.printerservice.manager.PrinterState.R_ORDERS;
...
public class PrinterManager implements DownloadInterface, RawPrintInterface {
    ...
    public int getRealStatus(int info) {
        ...
        switch (info) {
            ...
            case R_ORDERS:
                return printerState.orderCount;
            case R_HOTS:
                return printerState.hotCount;
            default:
                return -1;
        }
    }
        ...
        public void run() {
            while (printerState.initStatus == 3) {
                ...
                if (p == null) {
                    ...
                    //当打印异常时或MCU的buffer满时会阻塞不发打印数据直到状态恢复,但对未检测到黑标暂时放过
                    if (p.type == DataCell.PRINT_DATA && ((printerState.printerBufferCount <= 1 && printerState.current_status < INTERRUPT_TRANS) || (getPrinterState() != 1 && getPrinterState() != 9 && printerState.current_status < PROCESS_TRANS))) {
                        try {
                            Thread.sleep(50);
                        } catch (InterruptedException e) {
                            e.printStackTrace();
                        ...
                        if (p.series == SeriesTask.HEAD) {
                            ...
                            if (printerState.current_status == PROCESS_TRANS) {
                                ...
                                synchronized (printerState) {
                                    send__(getCommandbyte(C.CLEAR_BUFFER));
                                }
                                try {
                                    if (p.callback != null) {
    ...
    private void send__(byte[] d) {
        ...
        if (printerState.mess != 1) {
            ...
            printerState.bbm_flag = recv[130];
            printerState.bbm_value = (short) (((recv[131] & 0xFF) << 8) | (recv[132] & 0xFF));
