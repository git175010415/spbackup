...
public class WoyouServiceImpl extends IWoyouService.Stub implements RequestInterface {
    ...
    public void updataLocalDCB() {
        int tempDistance = p.getRealStatus(PrinterState.R_PRINTINGLENGTH);
        int tempCuts = p.getRealStatus(PrinterState.R_CUTTIMES);
        int tempOpens = p.getRealStatus(PrinterState.R_BOXTIMES);
        if(mServiceValue.addDistance(tempDistance) || mServiceValue.addCuts(tempCuts) || mServiceValue.addOpens(tempOpens)){
            PreferencesLoader pl = new PreferencesLoader(service, "settings");
            pl.saveInt("printer_length", mServiceValue.getDistance());
            pl.saveInt("printer_cut", mServiceValue.getCuts());
            pl.saveInt("printer_open", mServiceValue.getOpens());
        }
    }
